module mod_load
  ...
  contains
  subroutine load(io,filename,ng,nh,lo,hi,u,v,w,p,time,istep,po)
    !
    ...
    case('r')
      ...
      good = (product(ng)*4+2)*storage_size(1._rp)/8
      if(present(po)) good = good + product(ng)*storage_size(1._rp)/8
      ...
      call MPI_BCAST(fldinfo,2,MPI_REAL_RP,0,MPI_COMM_WORLD)
      time  =      fldinfo(1)
