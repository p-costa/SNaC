...
TARGET := $(NAME)
INPUT_FILES := dns.nml blocks.nml
...
run: $(EXE)
	@cp $(addprefix $(SRCS_DIR)/,$(INPUT_FILES)) $(RUN_DIR)
	@test ! -d $(SRCS_DIR)/grid || cp -r $(SRCS_DIR)/grid $(RUN_DIR)
	@printf "\nExecutable file $(TARGET), namelist files and optional grid folder copied to run folder $(RUN_DIR)\n"
